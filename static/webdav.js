...
const WebDAVNavigator = (url, options) => {
	...
	const rename_button = `<button class="btn btn-outline-success rename" type="button" data-bs-toggle="tooltip" data-bs-placement="top" title="rename" value="${_('Rename')}" ><i class="fs-4 fa fa-text-height"></i></button>`;
	const delete_button = `<button class="btn btn-danger delete" type="button" data-bs-toggle="tooltip" data-bs-placement="top" title="delete" value="${_('Delete')}" ><i class="fs-4 fa fa-trash"></i></button>`;


	const mkdir_dialog = `
	<div class="input-group mb-3">
  		<span class="input-group-text" id="basic-directory">@</span>
  		<input type="text" name="mkdir" placeholder="${_('Directory name')}" class="form-control" placeholder="Directory" aria-label="Directory" aria-describedby="basic-directory">
	</div>` ;
	const mkfile_dialog = `<input type="text" name="mkfile" class="form-control" placeholder="${_('File name')}" />`;
	const rename_dialog = `<input type="text" class="rename form-control" name="rename" placeholder="${_('New file name')}" />`;
	const paste_upload_dialog = `<h3>Upload this file?</h3><input type="text" class="form-control" name="paste_name" placeholder="${_('New file name')}" />`;
	const delete_dialog = `<p> Delete ?</p>`;
	...
	const dialog_tpl = `<dialog style="position: fixed;top: 1em;right: 1em;bottom: 1em;left: 1em;box-shadow: 1px 1px 5px black;background: white;border: 1px solid;boder-color: black;border-radius: .5em;" open>
	<div class="d-flex flex-row " style="flex-wrap: nowrap" ><p class="close"><input type="button" value="&#x2716;" class="btn btn-outline-dark close" /></p><form>%s%b</form></div></dialog>`;
	...
	const body_tpl = `<h1>%title%</h1>
		<div class="upload">
			<button class="btn btn-outline-success download_all" type="button" data-bs-toggle="tooltip" data-bs-placement="top" title="download all" value="${_('Download all files')}" ><i class="fs-4 fa fa-download"></i></button>
	...
	const create_buttons = `
			...
			<button class="btn btn-outline-success mkdir" type="button" data-bs-toggle="tooltip" data-bs-placement="top" title="mkdir" value="${_('New directory')}" ><i class="fs-4 fa fa-plus-circle"></i></button>
			<input type="file" style="display: none;" />
			<button class="btn btn-outline-success mkfile" type="button" data-bs-toggle="tooltip" data-bs-placement="top" title="new text file"  value="${_('New text file')}" ><i class="fs-4 fa fa-file-text-o"></i></button>
			<button class="btn btn-outline-success uploadfile" type="button" data-bs-toggle="tooltip" data-bs-placement="top" title="upload" value="${_('Upload file')}" ><i class="fs-4 fa fa-upload"></i></button>`;
	...
	const file_row_tpl = `<tr data-permissions="%permissions%" data-mime="%mime%" data-size="%size%" ><td class="thumb"><span class="icon %icon%"><b>%icon%</b></span></td><th><a href="%uri%">%name%</a></th><td class="size">%size_bytes%</td><td>%modified%</td><td class="buttons"><div>
	<a href="%uri%" download class="btn"><button class="btn btn-outline-success download_all" type="button" data-bs-toggle="tooltip" data-bs-placement="top" title="download" value="${_('Download')}" ><i class="fs-4 fa fa-download"></i></button></a>
	</div></td></tr>`;
	...
	const openDialog = (html, ok_btn = true) => {
		var tpl = dialog_tpl.replace(/%b/, ok_btn ? `<button class="btn btn-primary" type="submit" value="${_('OK')}">OK</button>` : '');
	...
	const buildListing = (uri, xml) => {
		...
		if (!root_permissions || root_permissions.indexOf('C') != -1 || root_permissions.indexOf('K') != -1) {
			...
			$('.mkfile').onclick = () => {
				openDialog(mkfile_dialog,false);
		...
		Array.from($('table').rows).forEach((tr) => {
			...
			if (!permissions || permissions.indexOf('NV') != -1) {
				...
				$$('.rename').onclick = () => {
					openDialog(rename_dialog,false);
